...
import {ContentLoader, SelectedItem, SuccessAlert, ErrorAlert} from '../components';
import { useError } from '../hooks';

const EDIT_PRODUCT = gql`
  mutation editProduct($id: ID, $title: String!, $description: String!, $price: Float, $images: [ImageInput], $created_At: Date, $color: [String]!, $sizes: [SizeQuantityInputs]!, $audience: Audience, $type: Producttype) {
  editProduct(
    product: {
      id: $id
      title: $title
...
const GET_PRODUCT = gql`
  query getProduct($id: ID!) {
    getProduct(id: $id) {
      title,description, images{url,alt}, price, color, sizes{size, quantity}, audience, type
    }
  }
`;
...
const AdminEditProduct = () => {
  ...
  const [editProduct, { loading, errors, data }] = useMutation(EDIT_PRODUCT, {
  ...
  const AddImage = async () => {
    if(!!IMGURL && !!IMGALT) {
      const newarr = images.map((e) => {
        return {
          url: e.url,
          alt: e.alt,
        }
      });
      await setImages([...newarr, {url: IMGURL, alt: IMGALT}]);
      await setIMGURL('');
      await setIMGALT('');
    }
  };
  ...
  useEffect(() => {
    if(!!getProduct.data) {
      console.log(getProduct.data.getProduct);
      const tempData = getProduct.data.getProduct;
      setAudience(tempData.audience);
      setColors(tempData.color);
      setDescription(tempData.description);
      setTitle(tempData.title);
      setImages(tempData.images.map((e) => {
        return {
          url: e.url,
          alt: e.alt,
        }
      ...
      setXS(tempData.sizes.find((e) => e.size === "XS").quantity);
      setS(tempData.sizes.find((e) => e.size === "S").quantity);
      setM(tempData.sizes.find((e) => e.size === "M").quantity);
      setL(tempData.sizes.find((e) => e.size === "L").quantity);
      setXL(tempData.sizes.find((e) => e.size === "XL").quantity);
      setXXL(tempData.sizes.find((e) => e.size === "XXL").quantity);
    };
    if(getProduct.loading) return(<ContentLoader />);
    if(!!getProduct.errors) console.log(errors);
  }, [getProduct.data, getProduct.loading, getProduct.errors, loading, data, errors]);


  return(
    <div className="admineditproduct container d-flex justify-content-center">
      {!!data && <SuccessAlert message="Product edited succesfully." />}
